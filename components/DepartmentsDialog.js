...
const DepartmentsDialog = ({ departments, onClose }) => {
  return (
    <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-20 flex justify-center items-center">
      <div className="w-5/6 md:w-4/5 bg-white">
        <div className="flex justify-between items-center border-b-2 border-grey-200 p-4">
          <span className="font-bold text-3xl">department</span>
          <a href="#" className="font-bold text-3xl" onClick={() => { event.preventDefault(); onClose(); }}>X</a>
        </div>
